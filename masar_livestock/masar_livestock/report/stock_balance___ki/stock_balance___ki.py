...
class StockBalanceReport:
	...
	def prepare_item_warehouse_map(self, item_warehouse_map, entry, group_by_key):
		...
		qty_dict.batch_no = entry.batch_no
		qty_dict.custom_initial_weight = entry.custom_initial_weight or 0
		qty_dict.custom_weight_remaining = entry.custom_weight_remaining or 0
		...
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				...
				"custom_initial_weight": entry.custom_initial_weight or 0,
				"custom_weight_remaining": entry.custom_weight_remaining or 0,
			}
		)
	...
	def prepare_stock_ledger_entries(self):
		...
		batch_bundle_table = frappe.qb.DocType("Serial and Batch Entry")
		...
		query = (
			...
			.left_join(batch_bundle_table)
			.on(batch_bundle_table.parent == sle.serial_and_batch_bundle)
			.left_join(batch_table)
			.on(batch_table.name == batch_bundle_table.batch_no)
			.select(
				sle.item_code,
				sle.warehouse,
				sle.posting_date,
				sle.actual_qty,
				batch_table.custom_initial_weight,
				batch_table.custom_weight_remaining,
				sle.valuation_rate,
				sle.company,
				sle.voucher_type,
		...
		columns.extend(
			[
				...
				},
				{
					...
					"fieldname": "custom_initial_weight",
					"fieldtype": "Float",
					"width": 120,
				},
				{
					"label": _("Remaining Weight"),
					"fieldname": "custom_weight_remaining",
					"fieldtype": "Float",
					"width": 140,
				},
				{
